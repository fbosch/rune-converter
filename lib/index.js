...
runeConverter.getRune = (string, variantIndex) => {
...
runeConverter.replaceLetterCombinations = string => {
  let combinations = _.keys(runeTable)
    .filter(key => key.length !== 1 && _.isString(runeTable[key]))
    .sort((a, b) => b.length - a.length)

  return combinations.reduce((str, key) => {
    return str = _.replace(str, key, runeConverter.getRune(key))
  }, _.toLower(_.clone(string)))
}


runeConverter.convert = (string, userSettings) => {
  const settings = _.assign({}, defaultSettings, userSettings)
  return runeConverter.replaceLetterCombinations(string).split('')
    .reduce((convertedString, letter) => {
      ...
      switch (letter) {
        ...
        default: {
          rune = runeConverter.getRune(letter)
        }
      }
