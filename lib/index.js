...
function transcribeSingleLetter(string) {
...
function transcribeLetterCombinations(string) {
  ...
  return combinations.reduce((str, key) => str = replace(str, key, transcribeSingleLetter(key)), toLower(clone(string)))
}
  ...
  return transcribeLetterCombinations(string).split('')
    .reduce((convertedString, letter) => {
      ...
      switch (letter) {
        ...
        default: {
          rune = transcribeSingleLetter(letter)
        }
      }
...
export { transcribeSingleLetter, transcribeLetterCombinations, transcribe, elderFuthark, transcriptionTable }
